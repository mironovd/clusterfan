...
class Seed:
    ...
    def __init__(self,n, vertices=None, graph=None, tabs=None, poly=None, schur=None):
	self.tabs = tabs or {}
	self.schur = schur or {}
        self.poly = poly or {}
        self.signs = {}
...
#	return map(lambda x: x[0:2], x.graph.outgoing_edges(vertex))
    ...
    def mutable(self):
	return [y for y in self.vertices if y[0]!=0 and y[0]+y[1]<self.deg-1 and len(self.In(y)+self.Out(y))<=args.valency]
#	return [y for y in self.vertices if y[0]!=0 and y[0]+y[1]<self.deg-1]
	...
	x=Seed(self.deg,vv,{e:m for e,m in z.items() if m!=0},a,pp,ss)
	x.signing()
	x.rehash()
	return x
